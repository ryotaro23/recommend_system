  ...
  const reactionButtons = Object.entries(reactionEmoji).map(([name, emoji]) => {
    return (
      <Button 
        ...
        variant="outlined" 
        color="primary" 
        onClick={() => dispatch(addReaction( post.id, name as ReactionName))}
      >
        <Typography fontSize="large">{emoji}</Typography>
        <Typography>{post.reactions[name as ReactionName]}</Typography>
      </Button>
    )
  })

  return <div style={{ display: 'flex', gap: '10px' }}>{reactionButtons}</div>
}
